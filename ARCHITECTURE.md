...
The manual guide editor (delivery-sequence item 3) has a first, minimal implementation: Draft Editor Foundation. It is deliberately narrower than a finished editor -- it covers creating a Guide and editing its Draft, not publishing or executing it (that is Publish and Knit, the next slice).
...
- Guides created and edited this way remain Draft-only (`GuideEntryAction.NOT_EXECUTABLE`) until published; this slice does not add a Publish action, so a newly authored Guide cannot yet be Started or Continued. That is Publish and Knit's job.
- This wiring was confirmed by inspection to exist and is backed by passing focused JVM tests (`DraftEditorViewModelTest`, plus new `ProjectDetailViewModelTest` coverage for guide creation) covering add/edit/delete/reorder for every node type, validation-failure and conflict recovery, and duplicate-submission guarding. Two production-navigation instrumented tests (adding a Guide, authoring a step, and reopening it after the fact; a Draft-only Guide's card opening the editor) were added and compile, but have not executed successfully in this environment -- blocked by the same pre-existing Espresso/emulator `InputManager.getInstance` incompatibility as every other Compose-based instrumented test here, not by anything specific to this wiring.
