...
- A Counters destination: create/edit/delete a counter with a name, a free-text unit label (rows, rounds, motifs, or any user-defined term), an optional goal, and an optional owning Project (or standalone); increment/decrement controls and a reset action (with confirmation) persist immediately, and counters are included in the JSON backup. Automatic reset rules, linked-counter behavior, repeating schedules, an active crafting screen, and persistent notifications are not built yet -- see ROADMAP.md Phase 3
...
- Room schema versioned through v7 with real migrations backing Guides, Definition Revisions, Executions, Projects, Library items, Stash items, Tool sets/items, and Counters
