...
- A Counters destination: create/edit/delete a counter with a name, a free-text unit label (rows, rounds, motifs, or any user-defined term), an optional goal, and an optional owning Project (or standalone); increment/decrement controls and a reset action (with confirmation) persist immediately. Automatic reset rules, linked-counter behavior, repeating schedules, an active crafting screen, persistent notifications, and JSON backup inclusion are not built yet -- see ROADMAP.md Phase 3
