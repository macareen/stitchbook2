...
- The first three increments of deterministic PDF pattern parsing: digital-text extraction with page/line source references (PdfBox-Android-backed `data/parsing/PdfBoxTextExtractor.kt`), deterministic parsing of that text into sections/row-round ranges/repeats with an ambiguity-issue list (`domain/parsing/PatternTextParser.kt`), and a "Create from PDF" action on a Project's guide list that maps the result into a real, editable, unpublished Guide Draft with provenance kept visible in the existing Draft editor -- OCR fallback for PDFs with no text layer does not exist yet, see ARCHITECTURE.md
...
- Guide authoring can now also start from a PDF's extracted, parsed text (see above), but OCR fallback for PDFs with no text layer does not exist yet.
