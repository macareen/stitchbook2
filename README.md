...
- The first two increments of deterministic PDF pattern parsing: digital-text extraction with page/line source references (PdfBox-Android-backed `data/parsing/PdfBoxTextExtractor.kt`), plus deterministic parsing of that text into sections/row-round ranges/repeats with an ambiguity-issue list (`domain/parsing/PatternTextParser.kt`) -- no Draft-editor mapping yet, see ARCHITECTURE.md
...
- Guide authoring is manual only -- PDF text extraction and deterministic section/row/repeat parsing exist (see above), but OCR fallback and mapping parsed output into a Draft do not yet.
