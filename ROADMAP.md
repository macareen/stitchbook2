...
**Status: Stash CRUD, CSV import/export, and storage/purchase/care/Ravelry-ID fields implemented; full skein/partial-skein tracking, measured-vs-estimated remaining length, and allocations/consumption against projects are not yet.** The Stash schema (name, category, brand, colorway, dye lot, weight category, fibre content, quantity, unit label, yardage/unit, notes, storage location, care instructions, Ravelry yarn ID, purchase source/price/date) supports full CRUD with search/category filtering, plus a documented, versioned CSV format (`data/csv/StashCsv.kt`, schema v2): export, a downloadable one-row template, and import with a pre-commit validation report (missing name, unrecognized category, non-numeric/negative quantity, yardage, or purchase price are all reported by row number without discarding the file's other valid rows) and duplicate handling by matching the `id` column (a matched id updates in place and preserves its original `createdAt`; blank or unmatched ids create new items). Stash items are included in the JSON safety export (`LocalBackupService`), so the six new fields round-trip there too. `storageLocation`/`careInstructions`/`purchaseSource`/`purchasePrice`/`purchaseDate` are meaningful for every category (you buy and store needles and notions too, not just yarn); `ravelryYarnId` is yarn-only, cleared for other categories the same way `colorway`/`dyeLot`/`weightCategory`/`fiberContent`/`yardagePerUnit` already were. `purchaseDate` is stored as a free-text ISO-8601 date string ("yyyy-MM-dd", ARCHITECTURE.md §9's "local dates for date-only concepts") rather than an epoch-millis instant, since a purchase date has no time-of-day meaning -- not format-validated on import or in the form, matching how every other free-text column in this format already works. Room v13->v14 added all six as plain nullable columns via `ALTER TABLE ADD COLUMN` (no foreign keys, `stash_items`' first schema change since `MIGRATION_3_4` created it). `StashViewModel.saveItem` and the add/edit dialog were refactored from an 11-parameter positional call into a `StashItemFormInput` data class (mirroring `ToolItemFormInput`) since six more positional parameters would have made the existing signature unreadable. Not yet implemented: full skein/partial-skein tracking, measured-vs-estimated remaining length, and allocations/consumption against projects (PRODUCT_SPEC.md §6.7's "Full and partial skein quantities," "Remaining measured weight," "Estimated remaining length," "Allocated and unallocated quantities," and "Associated projects" -- ARCHITECTURE.md §9's "projects-yarn lots" join entity) -- these are a larger, still-open modeling question (flat fields on `StashItem` vs. a per-skein "yarn lot" sub-entity) left for a separate future slice. Photos remain deferred to Phase 7 per this phase's non-goals.
