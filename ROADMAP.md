...
3. **Manual guide editor — Draft Editor Foundation in progress:** Let users create and review valid guide trees without parsing, preserve stable node identity, and prevent unsupported or invalid structures from entering execution. The first slice, Draft Editor Foundation, delivers Guide creation and Draft editing: a Project's "Add Guide" action creates a Guide (with its one empty Draft) and opens the Draft editor directly; the editor renders the Draft's node tree as a flattened outline supporting Section, Range, Repeat, and Instruction, with add/edit/delete/reorder each persisting immediately through `GuideRepository.saveDraft`. A structural save failure or optimistic-concurrency conflict surfaces a recoverable message and reloads the last known-good persisted state rather than crashing or silently discarding the edit, mirroring the reload-on-conflict shape Focus Mode already established for Executions. Guides created this way remain Draft-only until published; there is no Publish action yet, so a newly authored Guide cannot be Started or Continued. Publishing, publish-time validation feedback, and confirming Start/Focus Mode against a freshly published Guide are Publish and Knit, the next slice. This wiring is backed by passing focused JVM tests covering add/edit/delete/reorder for every node type, validation-failure and version-conflict recovery, and duplicate-submission guarding. Two production-navigation instrumented tests exercising Add Guide and the Draft-only entry point were added and compile, but have not executed successfully in this environment -- blocked by the same pre-existing Espresso/emulator incompatibility affecting every Compose-based instrumented test here, not by anything specific to this wiring.
